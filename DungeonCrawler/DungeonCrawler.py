...
if not exists('Settings.json'):
    os.open( 'Settings.json', os.O_RDWR|os.O_CREAT)
    open('Settings.json', 'w').close()
    with open('Settings.json', 'w') as file_handler:
        for item in Settings:
            file_handler.write('{}\n'.format(item))
        file_handler.write('\n# Delete this file for the orgin Settings\n# You may tweak this to your own story have fun!\n# Add the Game, Saves, Backups, and Settings into one directory and shortcut the game to your Desktop!')
else:
    with open('Settings.json', 'r') as fp:
      v = fp.read()
    e = v.split('\n')
    r = e[0].replace("'", '').replace('[', '').replace(']','').replace(' ', '').replace('\n', '').split(',')
    AllAttacks = r

    r = e[1].replace("'", '').replace('[', '').replace(']','').replace(' ', '').replace('\n', '').split(',')
    AttackHitPoints = r

    r = e[2].replace("'", '').replace('[', '').replace(']','').replace(' ', '').replace('\n', '').split(',')
    AttackHitPointsMax = r

    r = e[3].replace("'", '').replace('[', '').replace(']','').replace(' ', '').replace('\n', '').split(',')
    AttackTypes = r

    r = e[4].replace("'", '').replace('[', '').replace(']','').replace(' ', '').replace('\n', '').split(',')
    AttackRaritys = r

    r = e[5].replace("'", '').replace('[', '').replace(']','').replace(' ', '').replace('\n', '').split(',')
    BuyableAttacks = r

    r = e[6].replace("'", '').replace('[', '').replace(']','').replace(' ', '').replace('\n', '').split(',')
    BuyablePrices = r
    for i in BuyablePrices:
        index = BuyablePrices.index(i)
        BuyablePrices[index] = int(i)

    r = e[7].replace("'", '').replace('[', '').replace(']','').replace('\n', '').replace(' ', '').split(',')
    Names = r

    r = e[8].replace("'", '').replace('[', '').replace(']','').replace('\n', '').replace(' ', '').split(',')
    Aproaches = r

    r = e[9].replace("'", '').replace('[', '').replace(']','').replace('\n', '').replace(' ', '').split(',')
    Storys = r

    r = e[10].replace("'", '').replace('[', '').replace(']','').replace('\n', '').replace(' ', '').split(',')
    BossNames = r
...
def Save():
    ...
    os.open( 'DungeonSaves.json', os.O_RDWR|os.O_CREAT)

    open('DungeonSaves.json', 'w').close()
    with open('DungeonSaves.json', 'w') as file_handler:
        for item in Data:
            file_handler.write('{}\n'.format(item))
    Load()
def CreateBackup():
    ...
    if not exists('Backups'):
        os.mkdir('Backups')
    fileCount = len([name for name in os.listdir('Backups') if os.path.isfile(os.path.join('Backups', name))])
    os.open( f'Backups\DungeonSaves{fileCount + 1}.json', os.O_RDWR|os.O_CREAT)

    open(f'Backups\DungeonSaves{fileCount + 1}.json', 'w').close()
    with open(f'Backups\DungeonSaves{fileCount + 1}.json', 'w') as file_handler:
        for item in Data:
            file_handler.write('{}\n'.format(item))
    Load()
...
def Load():
    ...
    with open('DungeonSaves.json', 'r') as fp:
      v = fp.read()
    e = v.split('\n')
    LVL = int(e[0])
    w = float(e[1])
...
if not exists('DungeonSaves.json'):
    os.open( 'DungeonSaves.json', os.O_CREAT )
    with open('DungeonSaves.json', 'w') as file_handler:
        for item in Data:
            file_handler.write('{}\n'.format(item))
    Load()
                ...
                if(newerchoice in BuyableAttacks):
                    ...
                    if Coins >= BuyablePrices[index]:
                        ...
                        if(Coins < 0):
                            Coins = 0
                        Data = [LVL,EXP,Coins,Attacks,AttacksUpgrade,BossTimes]
                        choice = input(f'Would you like to Save and Reload? (Recommended, fixes items not working after bought) y/n\n')
                        if choice == 'y':
                            Save()
                            Load()
                elif 'save' in choice.lower():
                        Data = [LVL,EXP,Coins,Attacks,AttacksUpgrade,BossTimes]
                        Save()
